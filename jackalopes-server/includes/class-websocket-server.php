...
class Jackalopes_Server_WebSocket {
    ...
    public function __construct() {
        $this->server_script = JACKALOPES_SERVER_PLUGIN_DIR . 'includes/server/websocket-server.php';
    ...
    public function start() {
        ...
        // Make sure Composer autoloader is available
        if (!file_exists(JACKALOPES_SERVER_PLUGIN_DIR . 'vendor/autoload.php')) {
            $this->install_dependencies();
        }
        
        // Create server directory if it doesn't exist
        $server_dir = JACKALOPES_SERVER_PLUGIN_DIR . 'includes/server';
        if (!file_exists($server_dir)) {
            mkdir($server_dir, 0755, true);
        }
        
        // Create WebSocket server implementation if it doesn't exist
        if (!file_exists($this->server_script)) {
            $this->create_server_script();
        }
        ...
        if ($this->running) {
            ...
            $this->log_message('Failed to start server');
            return false;
        }
    }
...
PHP;
        ...
        file_put_contents($this->server_script, $script_content);
    }
} 